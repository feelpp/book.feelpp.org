...
[stem]
++++
\forall \epsilon \quad {\|u-u_h\|_{0,\Omega}} \leq {\hat C} {h^{k+1/2}} \|u\|_{k+1,\Omega}
++++

[[eq:stab30]]
[stem]
++++
\forall \epsilon \geq c h \quad {\|u-u_h\|_{1,\Omega}} \leq {\hat C} {h^{r}} \|u\|_{k+1,\Omega}
++++

and finally if the family stem:[\{\mathcal{T}_h\}_{h > 0}] is quasi-uniform and stem:[\epsilon \leq {\hat C} h ], then
[stem]
++++
\| \beta \cdot (\nabla u -u_h) \|_{0,\Omega} \leq {\hat C} h^k \|u \|_{k+1,\Omega}
++++
====
