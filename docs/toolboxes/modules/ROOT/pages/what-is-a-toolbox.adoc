...
[source,json]
.Basis of a `PostProcess` section
----
"PostProcess":
{
    "Exports":
...
It could be useful to compute easily the norm (L2/H1/Semi-H1) of a field or an error.
An example of json which define 2 norm computations. The results are stored in csv file in column named `Norm_mynorm_L2` and `Norm_myerror_L2-error`
[source,json]
...
An expression (scalar/vector/matrix) can be also passed to evaluate the norm. But in this case, the `field` entry must be removed.
[source,json]
----
"expr":"2*x*y:x:y"
----
In the case of  H1 or SemiH1 norm type, the grad_expr entry must be given.
[source,json]
----
"grad_expr":"{2*y,2*x}:x:y"
----

All solution/expression can be depends of specic symbol of the toolboxes used. For example, in the heat toolboxes : 
[source,json]
----
"expr":"2*heat_T+3*x:heat_T:x"
----
where `heat_T` is the  temperature solution computed at last solve.


The quadrature can be also given if an expression/solution is used. By default, the quadrature order is 5. For example,  use a quadrature order equal to 10 is done by adding :
[source,json]
----
"quad":10
----
