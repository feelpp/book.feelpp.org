...
[NOTE]
====
For *DevUser* using the {uri-msoportal-dev-www}{portal_test} ....
...
The {feelpp} offering is created by *DevUser* as described in this <<registerapp, section>>.
...
[[registerapp]]
===  Register an {feelpp} application
...
In this https://github.com/MSO4SC/resources/tree/master/blueprint/feelpp/feelpp_test[git repository]
you will find the actual application corresponding to the {feelpp} basic demonstrator.

To actually build the app, use: ```./deploy pkg```
====

[[validorder]]
=== Validate an {feelpp} application purchase order
...
into {feelpp} user group. Fortunately this need to be done only once for any {feelpp} app.
...
For app like {feelpp} apps that require an private singularity image, you must grant *EndUser* access to the {sregistry} {feelpp} team.
To do so follow these instructions:

* Logged in to your {uri-sregistry-cesga},
* Click on *Teams* menu then select your *team*

image::sregistry_team.png[ex: {{feelpp {sregistry} team]

* As in the image above, click on *Invite Users*

image::sregistry_team.png[ex: {{feelpp {invitation} team]
...
Then notify the owner/seller that you request to be part of {feelpp} user group.
...
NOTE: you need to ask the owner/seller of {feelpp} app to be granted the role of *editor*
...
Before proceeding to the setup of the app, you need to make sure, at least for {feelpp}
 app (or more generally for apps requiring access to a private singularity image) that you have
 been granted the proper right on {feelpp} {sregistry} team.
...
| *cfgfile*                     | configuration file                          | `/usr/share/doc/feelpp/ThermoElectricModel/quarter-turn3D.cfg` | cfg file need to be in the container, or in a mounted dir or in the dataset
|===
...
|===
| Key                        | Description              | Default          | Notes
| singularity_image_uri      |  URI pointing to the singularity image   | `feelpp/feelpp:stretch` |
| singularity_image_filename |  Filename of the singularity image       | `feelpp-stretch.simg` |
