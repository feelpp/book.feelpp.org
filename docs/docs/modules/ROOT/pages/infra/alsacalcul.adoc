...
WARNING: singularity is not yet officially ready on the cluster. A module should be soon be available.
`module av 2>&1 | grep singularity`

Get a xref:user:ROOT:install/containers.adoc[Feel++ singularity container].
...
Due to proprietary reason, the current Feel++ images provide only infiniband driver, therefore the cluster provides constraints to only use computing nodes that are either infiniband or omnipath interconnected nodes to have high performances. To use {feelpp} singularity container in you slurm script, you should add
[source]
----
...
modules load petsc/petsc-3.8.4.g63
