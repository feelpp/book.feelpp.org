...
----
$ tree -L 1 -d | column
.		├── cmake	├── databases	├── feelpp	├── ports	├── testsuite	11 directories
├── benchmarks	├── data	├── doc		├── mor		├── research	└── toolboxes
----
...
Compile {feelpp}, the {feelpp} toolboxes and {feelpp} mor
[source,sh]
.Listing compiling using clang++/libc++ in Debug mode
----
CXX=clang++ ../feelpp/configure -r --enable-toolboxes --enable-mor
----
