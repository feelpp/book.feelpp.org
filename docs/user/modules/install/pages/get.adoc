:feelpp: Feel++
= Get {feelpp}
:page-tags: manual, install-get
:page-cards-tag: manual
:page-illustration: fa-solid fa-download
:page-description: Get {feelpp}: ubuntu, debian, spack, docker, apptainer, Windows WSL
:parent-catalogs: install
...
echo "deb [signed-by=/etc/apt/keyrings/feelpp.gpg] http://apt.feelpp.org/{flavor}/{dist} {dist} {channel}" | sudo tee /etc/apt/sources.list.d/feelpp.list >/dev/null
sudo apt update
--
----
...
|===
|Distribution | Release | Version | Supported Channels | Comment
|debian | trixie | 13 | latest | LTS
|===

[discrete]
=== Trixie (13)

par::[debian,trixie,latest]
