...
\begin{equation*}
\left\{\begin{array}{l}
\text { Find } u_b \in L_{mathrm{c}, b}^k \text { such that } \\
a\left(u_b, w_h\right)=f\left(w_h\right), \quad \forall w_h \in V_b, \
\left.u_h\right|_{partial \Omega}=athcal{I}_b^{athrm{Lag} \partial} g .
\end{array}\right.
\end{equation*}
++++
...
++++
\begin{equation*}
\left\|u-u_b\right\|_{1, \Omega} \leqsant c h^k|u|_{k+1, \Omega} .
\end{equation*}
++++
