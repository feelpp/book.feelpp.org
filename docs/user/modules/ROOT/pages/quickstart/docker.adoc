...
> docker run -it<1>
  ...
  -v $HOME/feel:/feel <3>
  feelpp/feelpp-toolboxes <4>
----
<1> the option `-it` runs Docker in interactive mode;
<2> we need to pass your user id to Docker so that the data you generate from Docker belongs to you;
<3> maps the directory `$HOME/feel` on your host to `/feel` in Docker, {feelpp} write in `/feel` the results of the simulations. Passing your id, _i.e_ `LOCAL_USER_ID`, allows to ensure that the generated data on {feelpp} belongs to you on your host system;
<4> use the container `feelpp/feelpp-toolboxes`.
