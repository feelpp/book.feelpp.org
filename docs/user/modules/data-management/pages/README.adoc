...
Reduced Basis (CRB) basis. But also in the benchmark system (See <<_benchmark,Benchmark>> 
section for details).
These database are stored on a distant server or can be saved statically 
via DB dump.
...
[options="header"]
|===
| Type | format(s) | description

1.2+| <<_geometry_and_mesh,CAO files>>
| .geo 
| interpreted by GMSH 

| .step 
| interpreted by OpenCascade (and GMSH)

1.2+| <<_geometry_and_mesh,Mesh files>>
| .msh 
| the GMSH mesh format

|.h5+, .json
| the Feel++ mesh format


1.2+| <<_configuration_files,Configuration files>>
|.cfg : 
| {feelpp} configuration

| .json :
| {feelpp} model configuration

1.5+| <<_visualization,Visualization files>> _(ParaView)_
| Ensight format
| 
...
| VTK.js 
| vtkjs export file (used in documentation)

| .csv
| table of values

1.3+| <<_database,{feelpp} database>>
| .h5
| computation mesh, database, ...

| json
| journal files

| mongodb dump
| BSON/JSON based database dump file (journal, CRB,...)

1.2+| <<_documentation, Documentation files>>
| .adoc 
| documentation, man
