// -*- mode: adoc -*-
= Eigen Problem
:page-tags: manual, eigensolver
:page-illustration: fa-solid fa-bullseye
:description: Eigensolver strategies available in Feel++.
:page-cards-order: 4
...
[source,c++]
----
// assemble matrices A and B (see testsuite slepc example)
auto A = backend->newMatrix( _test = Xh, _trial = Xh );
auto B = backend->newMatrix( _test = Xh, _trial = Xh );
// compute eigenmodes
auto modes = SolverEigen<double>::eigs( _matrixA = A, _matrixB = B, _spectrum = SMALLEST_REAL, _transform = SINVERT );
double lambda = modes.begin()->second.get<0>();
auto eigen_vector = modes.begin()->second.get<2>();
----
