...
:page-tags: manual, preconditioners
:page-illustration: fa-solid fa-gears
:description: Preconditioner strategies available in Feel++.
:page-cards-order: 2
...
Tests in the repository show how to configure and use preconditioners
in practice. See `testsuite/feeldiscr/test_mult_petsc.cpp` and
`testsuite/feeldiscr/test_stokes.cpp` for concrete configurations and
use with iterative solvers. Typical options include `pc-type=jacobi`,
`pc-type=ilu`, and block or multigrid-based preconditioners configured
via the backend options.
