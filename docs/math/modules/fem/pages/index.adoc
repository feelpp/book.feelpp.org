...
= Introduction à la méthode des éléments finis
:page-layout: manuals
:page-tags: catalog,fem
:page-cards-tag: manual
:page-cards-within-module: true
:page-cards-title: Finite Element Method
...
\partial_n u=\partial u/\partial n=0& \hbox{ sur }\partial\Omega\\
\end{array}\right.
++++

Une _solution classique_ de ce problème est une fonction de stem:[{\cal C}^2(\bar{\Omega})] vérifiant (<<eq:modele-2D>>) en tout point de stem:[\Omega].
...
Prenons stem:[v=\varphi \in {\cal D}(\]a,b[)], alors
[stem]
++++
...
[stem]
++++
(-u''+cu,\varphi)_0 = (f,\varphi)_0\; \quad \forall \varphi \in {\cal D}(]a,b[).
++++
...
Une solution classique de stem:[({\cal P})], sous l’hypothèse que stem:[f\in{\cal C}^0(\bar{\Omega})] et stem:[g\in{\cal C}^0(\Gamma_1)], sera une fonction de stem:[{\cal C}^2(\bar{\Omega})] vérifiant l’équation en chaque point de stem:[\Omega].

La formulation variationnelle de stem:[({\cal P})] est obtenue par intégration par parties, elle s’écrit :
[[eq:FV3]]
[stem]
++++
({\cal Q})\quad
\left\{
\begin{array}{l}
\hbox{Trouver }u\in V = \left\{ v \in H^1(\Omega) \; , \; v=0 \hbox{ sur }\Gamma_0 \right\} \hbox{ tel que }\\
...
Pour obtenir une approximation numérique de stem:[u], on va maintenant remplacer l’espace stem:[V] qui est en général de dimension infinie par un sous-espace stem:[V_h] de dimension finie, et on va chercher à résoudre le _problème approché_
...
D’où l’existence et l’unicité de stem:[u_h], à nouveau par le théorème de
Lax-Milgram.
...
On a stem:[a(u,v)=l(v), \forall v\in V], donc en particulier stem:[a(u,v_h)=l(v_h), \quad \forall v_h\in V_h], car stem:[V_h\subset V].

Par ailleurs, stem:[a(u_h,v_h)=l(v_h), \quad \forall v_h\in V_h].

Par différence, on en déduit l'_orthogonalité de Galerkin_ :
[[eq:ortho-Galerkin]]
[stem]
++++
a(u-u_h,v_h)=0,\quad \forall v_h\in V_h   \label{eq:ortho-Galerkin}
++++
...
\end{array}
++++
Or stem:[v_h-u_h \in V_h].

Donc stem:[a(u-u_h,v_h-u_h)=0] d’après (<<eq:ortho-Galerkin>>).
...
Par ailleurs, stem:[a] étant continue, il existe stem:[M > 0] tel que
[stem]
++++
a(u-u_h,u-v_h)\le M \|u-u_h\| \, \|u-v_h\|.
++++
...
[stem]
++++
\|u-u_h\| \le \frac{M}{\alpha}\; \|u-v_h\| \quad \forall v_h\in V_h, M,\alpha > 0
++++
...
Soit stem:[(\varphi_1,\ldots,\varphi_{N_h})] une base de stem:[V_h]. En décomposant stem:[u_h] sur cette base sous la forme
...
On définira les fonctions de base par stem:[\varphi_i=1] sur le stem:[i^{ème}] ddl, et stem:[\varphi_i=0] sur les autres ddl.
...
== Retour à l’exemple 1-D (encore)

On reprend le problème 1-D <<eq:modele-1D>>. On a écrit sa formulation variationnelle <<eq:FV>> et montré qu’elle admet une solution unique dans la section (<<sec:modele-1D2>>).
...
.Définition
****
Les intervalles stem:[I_i] sont appelées _mailles_ ou _éléments_ ou _cellules_ du maillage, on a noté stem:[\Nma] le nombre de mailles
****
...
L’étape suivante est de choisir les _fonctions de forme_ ou _fonctions de base_ sur chaque maille.

On choisit les fonctions de stem:[V_h] tel que leur restriction sur chaque maille soit un _espace polynomial_.

[[def:24]]
.Définition: Espaces stem:[\Pk{k}]
****
Soit un entier stem:[k \leq 1].
En dimension 1, on appelle stem:[\Pk{k}] l’espace vectoriel des polynômes à coefficients réels de degré inférieur ou égal à stem:[k].
****
...
En effet les fonctions de stem:[w_h \in W_h] peuvent être discontinues aux interfaces entre les mailles et un résultat d’analyse fonctionnelle montre que dans ces conditions stem:[w_h \ni H^1(\Omega)].
...
++++
en d’autres termes, en dimension 1 (stem:[\Omega=[a,b\]]), on a
...
et le problème approché sur stem:[V_h] est :
[[eq:11]]
[stem]
++++
  ({\cal Q}_h)\qquad \hbox{Trouver } u_h\in V_h \hbox{ tel que } a(u_h,v_h)=l(v_h), \quad\forall v_h\in V_h
...
On s’intéresse à présent à des exemples concrets d’espaces d’approximations dans la section suivante <<sec:element-fini-lag>>.


[[sec:element-fini-lag]]
...
[[rem:7]]
IMPORTANT: Les fonctions stem:[(\varphi_i)_{i=1,...,\Nso}] satisfont les relations : stem:[\quad \varphi_i(x_j) = \delta_{ij}, i,j \in \{1,...,\Nso\},] où stem:[\delta_{ij}] désigne le symbole de Kronecker tel que stem:[\delta_{ij} = 1] si stem:[i=j] et stem:[\delta_{ij}=0] si stem:[i \neq j].
...
[[rem:8]]
IMPORTANT:  En dimension 1, les fonctions de stem:[H^1(\Omega)] sont _continues_, on peut donc voir stem:[\Ich{1}] comme un opérateur de stem:[H^1(\Omega)] dans stem:[H^1(\Omega)]. On montre qu’il est continu et que sa norme stem:[\|\Ich{1}\|_{\mathcal{L}(H^1(\Omega),H^1(\Omega))}] est uniformément bornée en stem:[h], c’est-à-dire qu’il existe une constante stem:[c], indépendante de stem:[h], telle que pour tout stem:[v \in H^1(\Omega)] : stem:[\quad \|\Ich{1} v \|_{1,\Omega} \leq c \|v\|_{1,\Omega}]]
...
[[sec:element-fini-Pk]]
=== Element fini de Lagrange stem:[P^k]
...
On montre qu’il est continu et que sa norme stem:[\|\Ich{k}{v}\|_{\mathcal{L}(H^1(\Omega),H^1(\Omega))}] est uniformément bornée en stem:[h], c’est-à-dire qu’il existe une constante stem:[c], indépendante de stem:[h] mais dépendante de stem:[k], telle que pour tout stem:[v \in H^1(\Omega)], stem:[\quad \|\Ich{k}{v} \|_{1,\Omega} \leq c \|v\|_{1,\Omega}]
...
[[rem:5]]
IMPORTANT: On dit que l’estimation d’erreur <<eq:16>> est _optimale_ car elle est du même ordre que l’erreur d’interpolation en norme stem:[L^2], voir la <<prop:2>>.

[[sec:form-algebr]]
== Formulation algébrique stem:[V_h=P_{c,h}^1]

En décomposant la solution approchée stem:[u_h] sur cette base sous la forme stem:[{u_h = \sum_{i=1}^N \mu_i \; \varphi_i}], on obtient, comme au paragraphe <<sec:fem-overview>>, le système linéaire stem:[A\mu=b], avec :
[stem]
++++
\begin{array}{rcl}
...
1.  Dans la section <<sec:lax-milgram>>, montrer que, dans le cas où
...
6.  Pour le problème 2-D de la section <<sec:modele-2D>>, montrer que la formulation
variationelle (<<eq:FV2>>) admet une solution unique, qui est aussi
solution classique si stem:[f \in H^2(\Omega)].
7.  Démontrer les résultats de la section <<sec:elliptique>>
