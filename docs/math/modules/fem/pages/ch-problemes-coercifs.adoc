...
On note stem:[\disp{d}: \Omega \rightarrow \mathbb{R}^3] le déplacement de la structure induit par cette charge stem:[\mathbf{f}].

En supposant que les déformations soient petites pour être modélisées dans le cadre de l’elasticité linéaire, on a la relation suivante à l’équilibre des forces:
...
++++
où stem:[\stresst{\disp{d}}: \Omega \rightarrow \RR^{d\times d}] est le _tenseur des contraintes_ défini par la relation
[[eq:93]]
[stem]
++++
\stresst{\disp{d}} = \lambda \tr(\deformt{\disp{d}}) \Id + 2 \mu \deformt{\disp{d}}
++++
et stem:[\deformt{\disp{d}} : \Omega \rightarrow \RR^{d\times d}] est  le _tenseur des déformations_ défini par
[[eq:94]]
[stem]
++++
