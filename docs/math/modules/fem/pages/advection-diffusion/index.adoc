...
On note stem:[p = \essinf_{x \in \Omega} (\mu -\frac{1}{2} \nabla \cdot \mathbf{\beta})] et stem:[c_\Omega] est la constante de l’inégalité de Poincaré xref:math:analyse-fonctionnelle:lemma.adoc#lemmaB61[Lemma B61].
...
++++
En posant stem:[c=\min(0,\frac{p}{c_\Omega}] et en utilisant l'inégalité de Poincaré <<B61>> on a
[stem]
++++
