...
++++
\begin{align*}
\Lambda\mathbf{u} + \nabla p - \Lambda \alpha p  &= \mathbf 0 & &\text{in }\Omega & &(1)\\
...
* stem:[\Omega\subset\mathbb R^n] is a polyhedral domain stem:[(n\geq 2)], 
* stem:[\partial\Omega = \Gamma = \Gamma_D \cup \Gamma_N], 
* stem:[d(\mathbf x)] is a scalar nonnegative function, 
* stem:[\Lambda(\mathbf x)] is a matrix valued function that is symmetric and uniformly positive definite on stem:[\Omega], 
* stem:[\alpha] is a vector valued function corresponding, also named the convection velocity, and 
* stem:[f\in L^2(\Omega)]. 

NOTE: If stem:[d = 0], we get the Darcy model presented in <<darcy-model>>. These assumptions can be generalized but we take them to simplify the discussion.

The Darcy problem presented in <<stabilized-formulations>> fits into this framework. Here, rather than using stabilized Galerkin formulations, the idea is to insert a Lagrange multiplier to handle the continuity of the normal components of the approximated flux stem:[\mathbf{u}_h]. 
In other words, the requirement stem:[\mathbf{u}_h\in H(\text{div},\Omega)] will be written in weak form initially, and then recovered in strong form later.
...
++++
\begin{align*}
\Lambda\mathbf u + \nabla p - \Lambda \alpha p &= \mathbf 0 & &\text{in }K,\\
...
The transmission conditions state that the normal component of stem:[\hat{\mathbf u}] across interelement boundary must be continuous. 

We can obtain stem:[(\mathbf u, p)] in stem:[K] in terms of stem:[\hat p] on stem:[\partial K] and stem:[f] by solving the following _local_ Dirichlet problem
[stem]
++++
\begin{align*}
\Lambda\mathbf u + \nabla p - \Lambda \alpha p &= \mathbf 0 & &\text{in }K, & &(5)\\
...
++++
\begin{align*}
(\Lambda\mathbf u_h,\mathbf v)_K - (p_h, \nabla\cdot\mathbf v)_K - (\Lambda \alpha p_h,  \mathbf v)_K + \langle\hat p_h, \mathbf v\cdot\mathbf n\rangle_{\partial K} &= 0, & &(11)\\
-(\mathbf u_h, \nabla w)_K + \langle\hat{\mathbf u}_h\cdot\mathbf n,w\rangle_{\partial K} + (dp_h, w)_K &= (f,w)_K, & &(12)
\end{align*}
++++
for all stem:[(\mathbf v, w) \in \mathbf V(K)\times W(K)], where stem:[\hat{\mathbf u}_h] is the numerical trace of the flux and depends, in general, on stem:[\mathbf u_h, p_h], and stem:[\hat p_h]. 

We use the the divergence theorem to rewrite the first equation to integrate by parts.
[stem]
++++
\begin{align*}
(\nabla p_h, \mathbf v)_K & = - (p_h, \nabla\cdot\mathbf v)_K + \langle\hat p_h, \mathbf v\cdot\mathbf n\rangle_{\partial K} 
\end{align*}
++++

NOTE: Different methods will correspond to different choices of stem:[\hat{\mathbf u}_h].
...
++++
\begin{align*}
(\Lambda\mathbf u_h,\mathbf v)_{\Omega_h} - (p_h, \nabla\cdot\mathbf v)_{\Omega_h}  - (\Lambda \alpha p_h,  \mathbf v)_K + \langle\hat p_h, \mathbf v\cdot\mathbf n\rangle_{\partial\Omega_h} &= 0 & &\forall\mathbf v\in \mathbf V_h, & &(16)\\
...
++++
\begin{align*}
&(\Lambda\mathbf u_h,\mathbf v)_{\Omega_h} & &- (p_h, \nabla\cdot\mathbf v)_{\Omega_h}- (\Lambda \alpha p_h,  \mathbf v)_K  & &+ \langle\hat p_h, \mathbf v\cdot\mathbf n\rangle_{\partial\Omega_h} & &= 0, & &(21)\\
...
++++
\begin{align}
A_{11}^K &\leftrightarrow (\Lambda\mathbf u_h,\mathbf v)_K, & &A_{12}^K\leftrightarrow- (p_h, \nabla\cdot\mathbf v)_K- (\Lambda \alpha p_h,  \mathbf v)_K, & &A_{13}^K\leftrightarrow\langle\hat p_h, \mathbf v\cdot\mathbf n\rangle_{\partial K},\\
